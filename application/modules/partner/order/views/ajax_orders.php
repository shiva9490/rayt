...
?>
<div class="col-md-12 mt-5 table-responsive t_div">
        ...
        <tbody>
            ...
            <tr>
                <td id="orders<?php echo $ve->order_unique_id;?>" data-types<?php echo $ve->order_unique_id;?>="<?php echo $orders;?>" data-status<?php echo $ve->order_unique_id;?>="<?php echo $ve->orderdetails_rest_staus;?>">
                    <a class="orders<?php echo $ve->order_unique_id;?>" onclick="orderModel('<?php echo $ve->order_unique_id;?>')" >
                ...
                <td>
                    <?php 
                        $par['whereCondition'] ="order_details_id LIKE '".$ve->orderdetails_id."'";
                        $res  = $this->order_model->viewOrderasson($par);
                        $variantsamount ='0';
                        if(is_array($res) && count($res) > 0){
                            $d = array();
                            foreach($res as $key=>$r){
                                $variantsamount = $variantsamount+$r->orderassons_amount;
                            }
                        }
                        echo number_format((float)$variantsamount+$ve->orderdetail_price, 3, '.', '');
                    ?>
                </td>
